...
<template>
  ...
  >
    ...
    >
      <template #left>
        <list-list-with-form-example
          :dummy-examples="dummyExamples"
          :checked-all="checkedAll"
          :checked-data="checkedData"
          @handleCheckedAll="handleCheckedAll"
          @handleDelete="handleDelete"
          @checked="addToCheckedList"
          @onChangeSelectedValue="onChangeSelectedValue"
        />
      </template>
      <template #right>
        <form-list-with-form-example
          :selected-product="selected"
          :status="status"
          @editForm="editForm"
          @cancelWriteForm="cancelWriteForm"
        />
      </template>
    </two-grid-split-view>
  </v-card>
...
export default class ListWithFormExample extends Vue {
   ...
   * 메인데이터를 로드해주는 곳
   */
  private async loadDummyData () {
    const res = await (axios.get('https://fakestoreapi.com/products'))
    this.dummyExamples = res.data
    console.log(this.dummyExamples)
  }

  private onChangeSelectedValue (selectedData: DummyProduct) {
    this.status = 'READ'
    this.selected = _.cloneDeep(selectedData)
  }
  ...
  private cancelWriteForm () {
    this.status = 'READ'
    const found = this.dummyExamples.find(data => data.id === this.selected.id)
    this.selected = _.cloneDeep(found || {} as DummyProduct)
  }

  private addToCheckedList (bool: boolean, data: DummyProduct) {
    if (bool) this.checkedData.push(data)
    else {
      this.checkedData.removeOne(data)
    }
    this.checkedAll = this.checkedData.length === this.dummyExamples.length
  }

  private handleCheckedAll (bool: boolean) {
    if (bool) this.checkedData = _.cloneDeep(this.dummyExamples)
  ...
  private handleDelete () {
    if (!confirm('선택한 값들을 모두 삭제하시겠습니까?')) return
  }
}
</script>
